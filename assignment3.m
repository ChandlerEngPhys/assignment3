...
Voltage = 0.8;%Volts
E_field = Voltage/xlimit(2);
Force = q*E_field;
acceleration = Force/m_n;
concentration = 10e15;
...
J = zeros([1 timesteps]);%Drift current
...
for t = 0:dt:tr
    ...
    if part2
        ...
        for p = 1:particles
            if Pscat > rand
                ...
                Vx(p) = Rx*normalized(1) + acceleration*dt;
    ...
    Time(n) = t;
    %Save the drift current at this time
    V_avg(n)=mean(Vx);
...
%% Assignment 3 Question 1D: Current Density vs Time
figure;
J = q*concentration*V_avg;
plot(Time,J,'-')
xlabel('Time (s)');
ylabel('Current (A)');
